...
- **0.9.5**：根治"更新就要重配对"——桌面启动时把私网 IPv4 的局域网中继端点
  ...
  探测到"有下行无上行"（半双工假在线）25 秒即主动换路，不再干等链路。
- **0.9.4**：撤掉 Apple 家族的滚动边缘模糊带（残影观感），玻璃面板本体保留。
- **0.9.3**：Apple 家族一轮修调（破坏性按钮/徽章去暖色、iOS 绿开关、玻璃材质增强）；
  桌面会话「停止」改为图标钮；**安卓调试 APK 固定签名**——本机与 CI 产物从此互相覆盖
  安装，CI 与发布门禁都会校验签名指纹。注：0.9.2 及更早的 CI 签名包装机需先卸载一次。
